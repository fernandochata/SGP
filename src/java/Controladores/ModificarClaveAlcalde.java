...
@WebServlet(name = "ModificarClaveAlcalde", urlPatterns = {"/ModificarClaveAlcalde"})
public class ModificarClaveAlcalde extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(usuarioDTO.getClave().equals(claveActual)){ // CLAVE ACTUAL ES CORRECTA
                if(claveNueva1.equals(claveNueva2)){ // CLAVES INGRESADAS SON IGUALES
                    usuarioDTO.setClave(claveNueva1);
                    if(usuarioDAO.update(usuarioDTO)){ // SE MODIFICÓ CORRECTAMENTE
                    ...
                    }else{ // ERROR AL MODIFICAR CLAVE DE USUARIO
                        String mensajeError = "Se produjo un error al actualizar la clave.";
                        request.getSession().setAttribute("mensajeError", mensajeError);
                        request.getRequestDispatcher("cambiarClaveAlcalde.jsp").forward(request, response);  
                    }
                }else{ // CLAVES INGRESADAS NO SON IGUALES
                    String mensajeError = "Las claves no coinciden. Inténtelo nuevamente.";
                    request.getSession().setAttribute("mensajeError", mensajeError);
                    request.getRequestDispatcher("cambiarClaveAlcalde.jsp").forward(request, response);     
                }
            }else{ // CLAVE ACTUAL ES INCORRECTA
                String mensajeError = "Clave ingresada no corresponde a la clave almacenada, intente de nuevo.";
                request.getSession().setAttribute("mensajeError", mensajeError);
                request.getRequestDispatcher("cambiarClaveAlcalde.jsp").forward(request, response);
            }
        } catch(NullPointerException ex) {
            String mensajeError = "Error inesperado (ModificarClaveAlcalde) | " + ex.getMessage();
            request.getSession().setAttribute("mensajeError", mensajeError);
